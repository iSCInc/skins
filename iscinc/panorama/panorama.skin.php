...
class SkinPanorama extends SkinTemplate {
	...
	public $skinname = 'panorama', $stylename = 'panorama',
	...
	function setupSkinUserCss( OutputPage $out ) {
		...
		$baseCSSmodules = array( 'skins.panorama' );
		...
		if ( version_compare( $wgVersion, '1.23c', '<' ) ) {
			$baseCSSmodules[] = 'skins.panorama.externallinks.123';
		} elseif (
		...
		)
		{
			...
			$baseCSSmodules[] = 'skins.panorama.externallinks.124';
		}
		...
		if ( $title->isMainPage() ) {
			$baseCSSmodules[] = 'zzzskins.panorama.mainpage';
		}
		...
		switch ( $action ) {
			case 'delete':
				$modules[] = 'skins.panorama.hacks.action.delete';
				break;
			case 'edit':
			case 'submit': // action=edit after previewing etc.
				$modules[] = 'skins.panorama.hacks.action.edit';
				break;
			case 'history':
				$modules[] = 'skins.panorama.hacks.action.history';
				break;
			case 'protect':
				$modules[] = 'skins.panorama.hacks.action.protect';
				break;
			default:
				break;
		}
		...
		if ( $title->inNamespace( NS_FILE ) ) {
			$modules[] = 'skins.panorama.hacks.filepage';
		} elseif ( $title->inNamespace( NS_SPECIAL ) ) {
			switch ( strtolower( $title->getDBkey() ) ) {
				case 'log':
					$modules[] = 'skins.panorama.hacks.special.log';
					break;
				case 'movepage':
					$modules[] = 'skins.panorama.hacks.special.movepage';
					break;
				case 'recentchanges':
					$modules[] = 'skins.panorama.hacks.special.recentchanges';
					break;
				case 'undelete':
					$modules[] = 'skins.panorama.hacks.special.undelete';
					break;
				case 'watchlist':
					$modules[] = 'skins.panorama.hacks.special.watchlist';
					break;
				default:
					break;
			}
		...
		if ( !function_exists( 'wfDisplayTheme' ) ) {
			...
			$themeModule = 'themeloader.skins.panorama.blue';
			// The 'themeloader.' prefix is a hack around
			// https://bugzilla.wikimedia.org/show_bug.cgi?id=66508
			if ( $theme && isset( $wgResourceModules['themeloader.skins.panorama.' . $theme] ) ) {
				$themeModule = 'themeloader.skins.panorama.' . $theme;
			} elseif ( isset( $wgDefaultTheme ) && $wgDefaultTheme != 'default' ) {
				$themeModule = 'themeloader.skins.panorama.' . $wgDefaultTheme;
			} elseif (
			...
			$out->addModuleStyles( 'themeloader.skins.panorama.blue' );
		}
	}
	...
	function getUserLinks() {
		...
		if ( $t->exists() ) {
			...
			if ( $text != '' ) {
				$ret = '<h3>' . $this->msg( 'panorama-mylinks' )->escaped() . '</h3>';
	...
	function getTabsArray( $showArticleTabs ) {
		...
		if ( $title->getNamespace() != NS_CATEGORY ) {
			...
			if ( in_array( $title->getNamespace(), array( NS_USER, NS_USER_TALK ) ) ) {
				...
				$msg = $this->msg( 'panorama-discuss' )->plain();
			}
		...
		if ( $user->isAllowedAny( 'delete', 'protect', 'unprotect', 'move' ) && $title->userCan( 'delete' ) ) {
			$adminTab->href = '#';
			$adminTab->text = $this->msg( 'panorama-admin-menu' )->plain();
			$adminTab->class = '';
	...
	function genNavigationTabs() {
		...
		$navTabs = array(
			'nav_messages' => array(
				...
				'link' => '#',
				'text' => $this->msg( 'panorama-navbar-messages' )->plain()
			),
			'nav_profile' => array(
				'menu' => $sk->getHeaderMenu( 'profile' ),
				'link' => $isLoggedIn ? $user->getUserPage()->getLocalURL() : '#',
				'text' => $isLoggedIn ? strtoupper( $this->msg( 'panorama-navbar-profile' )->plain() ) : strtoupper( $this->msg( 'login' )->plain() )
			),
			'nav_explore' => array(
				'menu' => $sk->getHeaderMenu( 'explore' ),
				'link' => '#',
				'text' => $this->msg( 'panorama-navbar-explore' )->plain()
			),
			'nav_help' => array(
				'menu' => $sk->getHeaderMenu( 'help' ),
				'link' => '#',
				'text' => $this->msg( 'panorama-navbar-help' )->plain()
			)
		);
	...
	function getHeaderMenu( $menu ) {
		...
		switch ( $menu ) {
			case 'edit':
				$html = Linker::link(
					$title,
					$this->msg( 'panorama-edit-this-article' )->plain(),
					array(),
					$sk->editUrlOptions()
				);
				if ( !$isLoggedIn ) {
				...
				if ( $isLoggedIn ) {
					...
					if ( class_exists( 'EchoEvent' ) ) {
						$myNotifications = Linker::link(
							...
							$this->msg( 'panorama-my-notifications' )->plain()
						);
					}
					$html = Linker::link( Title::makeTitle( NS_SPECIAL, 'Mytalk', 'post' ), $this->msg( 'mytalk' )->plain() ) .
							Linker::link( SpecialPage::getTitleFor( 'Mypage' ), $this->msg( 'panorama-my-page' )->plain() ) .
							$myNotifications .
			...
			case 'explore':
				...
				if ( !$isLoggedIn ) {
					$html .= Linker::link(
						...
						$this->msg( 'panorama-navmenu-aboutus' )->text()
					);
				}
				$html .= Linker::link( SpecialPage::getTitleFor( 'Categories' ), $this->msg( 'panorama-navmenu-categories' )->text() ) .
			...
			case 'help':
				...
				$html .= Linker::link(
					...
					$this->msg( 'panorama-categorize' )->plain()
				);
				$html .= Linker::link(
					Title::newFromText( $this->msg( 'panorama-more-ideas-url' )->text() ),
					$this->msg( 'panorama-more-ideas' )->plain()
				);
				break;
			case 'messages':
						...
						if ( $this->notifications_count > $maxNotesShown ) {
							$unshown = '<br />' . Linker::link(
								...
								$this->msg( 'parentheses',
									$this->msg( 'panorama-unread-notifications' )->numParams(
		...
		if ( !is_array( $box ) ) {
			...
			if ( $user->getNewtalk() ) {
				...
				$msg = '<div class="note_row"><div class="note_icon_talk"></div>' .
					Linker::link(
						$user->getTalkPage(),
						$this->msg( 'panorama-notifications-new-talk' )->numParams( $talkCount )->parse()
					) . '</div>';
				$notes[] = $msg;
				$newTalk = true;
			} else {
		...
		if ( $html ) {
			...
			if ( !$newTalk ) {
				$html .= '<div class="note_row note_empty">' .
					$this->msg( 'panorama-notifications-no-talk', $talkPage )->parse() .
					'</div>';
			}
		} else {
			$html = '<div class="note_row note_empty">' .
				$this->msg( 'panorama-notifications-none', $talkPage )->parse() .
				'</div>';
		}
	...
	static function getHolidayLogo() {
		...
		$holidayLogos = array(
			array(
				'logo' => $wgStylePath . '/panorama/resources/images/wikihow_logo_halloween.png',
	...
	public function getPageLastEdit( Title $title ) {
		...
		if ( $title->exists() && in_array( $title->getNamespace(), $wgContentNamespaces ) ) {
			...
			if ( $revision instanceof Revision ) {
				...
				if ( $author ) {
					$msg = $this->msg( 'panorama-page-edited-user', $formattedTS, $author )->parse();
				} else {
					$msg = $this->msg( 'panorama-page-edited', $formattedTS )->parse();
				}
			}
		}
...
class panoramaTemplate extends BaseTemplate {
	...
	/**
	 * Template filter callback for panorama skin.
	 * Takes an associative array of data set from a SkinTemplate-based
	 * class, and a wrapper for MediaWiki's localization database, and
	 * outputs a formatted page.
	 */
	public function execute() {
		global $wgStylePath, $wgSitename, $wgForumLink, $panoramaTOC;

		$tocHTML = '';
		if ( $panoramaTOC != '' ) {
			if ( sizeof( $panoramaTOC ) > 6 ) {
				$tocHTML .= "<div class='toc_long'>";
			} else {
				$tocHTML .= "<div class='toc_short'>";
			}
			$i = 0;
			foreach ( $panoramaTOC as $tocpart ) {
		...
		)
		{
			...
			$editLinkText = ( $title->getNamespace() == NS_MAIN ) ? $sk->msg( 'panorama-edit-page' )->plain() : $sk->msg( 'edit' )->plain();
		...
		$showFollowWidget = !$sk->msg( 'panorama-follow-table' )->isDisabled();
		...
		<div id="header_outer"><div id="header">
			...
			<?php endif; ?>
			<?php
				$holidayLogo = Skinpanorama::getHolidayLogo();
				$customLogo = wfFindFile( 'panorama-logo.png' );
				/**
		...
		<div id="article"<?php if ( class_exists( 'Microdata' ) ) { echo Microdata::genSchemaHeader(); } ?> class="mw-body">
			...
			<div class="section sticky" id="intro">
				...
				<?php if ( $tocHTML != '' ) { ?>
				<div id="header_toc">
					<span id="header-toc-header"><?php echo $sk->msg( 'panorama-toc-sections' ) ?></span><?php echo $tocHTML; ?>
				</div>
				<?php } ?>
			</div>
			...
			if ( in_array( $title->getNamespace(), array( NS_MAIN, NS_PROJECT ) ) && $action == 'view' ) {
				...
				<div class="section noprint">
					<?php if ( $showingArticleInfo ): ?>
						<h2 class="section_head" id="article_info_header"><span><?php echo $sk->msg( 'panorama-article-info' )->plain() ?></span></h2>
						<div id="article_info" class="section_text">
					<?php else : ?>
						<h2 class="section_head" id="article_tools_header"><span><?php echo $sk->msg( 'panorama-article-tools' )->plain() ?></span></h2>
					...
					<?php endif ?>
						...
						?>
						<ul id="end_options">
							<li class="endop_discuss"><span></span><a href="<?php echo $talk_link ?>" id="gatDiscussionFooter"><?php echo $sk->msg( 'panorama-discuss' )->plain() ?></a></li>
							...
							*/
							...
							<?php if ( $isLoggedIn ) {
								...
								// i18n messages used here: panorama-watch,
								// panorama-unwatch
								?>
									<li class="endop_watch"><span></span><a class="mw-watchlink" href="<?php echo $title->getLocalURL( 'action=' . $watchAction ); ?>"><?php echo $sk->msg( 'panorama-' . $watchAction )->plain() ?></a></li>
							<?php } ?>
		...
		?>
			<div id="sidebar">
				...
				$writeMsgContents = $sk->msg( 'panorama-write-article-url' )->text();
				if ( preg_match( '/^(?:' . wfUrlProtocols() . ')/', $writeMsgContents ) ) {
					$write = Linker::makeExternalLink(
						$writeMsgContents,
						$sk->msg( 'panorama-write-article' )->plain(),
						/* escape */true,
						/* link type */'',
					...
					<div id="side_featured_contributor" class="sidebox">
						...
						<?php if ( !$isLoggedIn ) { ?>
							<p class="bottom_button"><?php echo Linker::link(
								SpecialPage::getTitleFor( 'Userlogin' ),
								$sk->msg( 'panorama-featured-contributor-action' )->plain(),
				...
				<?php if ( $showFollowWidget ) { ?>
					<div class="sidebox">
						<h3><?php echo $sk->msg( 'panorama-follow-header' )->plain() ?></h3>
						<?php echo $sk->msg( 'panorama-follow-table' )->text() ?>
					</div>
				<?php } ?>
			</div><!--end #sidebar-->
		...
		<div id="footer_outer">
			<div id="footer">
				<div id="footer_side">
					<?php
						if ( $isLoggedIn ) {
							$footerMessage = $sk->msg( 'panorama-site-footer' );
						} else {
							$footerMessage = $sk->msg( 'panorama-site-footer-anon' );
						}
...
<?php
	 ...
	 * [[MediaWiki:panorama-featured-contributor-list]] has some content.
	 *
	...
	private function showFeaturedContributorWidget( $top = false ) {
		...
		$sk = $this->getSkin();
		$msg = $sk->msg( 'panorama-featured-contributor-list' )->inContentLanguage();
		if ( $msg->isDisabled() ) {
...
?>
	<div>
		<h3><?php echo $sk->msg( 'panorama-featured-contributor-title' )->plain(); ?></h3>
